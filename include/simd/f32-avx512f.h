...
#define XNN_SIMD_STATIC_INIT_U32(var, val) \
  const __m512i var = _mm512_set1_epi32(val);
#else
#define XNN_SIMD_STATIC_INIT_F32(var, val)                                     \
  static const xnn_simd_f32_t var = {(val), (val), (val), (val), (val), (val), \
                                     (val), (val), (val), (val), (val), (val), \
                                     (val), (val), (val), (val)};
#define XNN_SIMD_STATIC_INIT_U32(var, val)                              \
  static const __m512i var = {                                          \
...
XNN_INLINE xnn_simd_f32_t xnn_zero_f32() { return _mm512_setzero_ps(); }
...
XNN_INLINE xnn_simd_f32_t xnn_set1_f32(float v) { return _mm512_set1_ps(v); }
