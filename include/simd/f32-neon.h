...
// Static initialization of the SIMD types does not seem to work in MSVC, so
// using `_mm_set1_ps`/`_mm_set1_epi32` instead. Since these are not considered
// static initializers by the compiler, the variable itself cannot be `static`,
// otherwise the compiler will try to guard it (expensive). Looking at the
// generated code, the creation of the `const` variables is moved outside of the
// inlined functions, i.e. the registers are re-used.

#define XNN_SIMD_STATIC_INIT_F32(var, val) \
  const float32x4_t var = _mm_set1_ps(val);

#define XNN_SIMD_STATIC_INIT_U32(var, val) \
  const uint32x4_t var = _mm_set1_epi32(val);
#else
#define XNN_SIMD_STATIC_INIT_F32(var, val) \
  static const float32x4_t var = {(val), (val), (val), (val)};
