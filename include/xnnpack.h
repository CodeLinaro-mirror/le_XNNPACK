...
struct xnn_weights_cache_provider {
  ...
  /// Release any allocated memory hosted by context. Note it should not deallocate
  /// context itself.
  /// @param context - The user-specified pointer from xnn_weights_cache_provider structure.
  enum xnn_status (*release)(void* context);

  /// Looks up the tuple of {cache_key, kernel, bias} in the cache. If it is found,
  /// returns the pointer to the found entry for reuse. Otherwise, returns NULL.
  /// @param context - The user-specified pointer from xnn_weights_cache_provider structure.
  /// @param cache_key - The unique key for each ukernel. It is guaranteed that each ukernel provides
  ///                    a consistent and identical key.
  /// @param kernel - Pointer to the original kernel.
  /// @param bias - Pointer to the original bias, could be NULL.
  size_t (*look_up)(void* context, size_t cache_key, const void* kernel, const void* bias);
  ...
  /// Otherwise, it is added to the cache.
  /// @param context - The user-specified pointer from xnn_weights_cache_provider structure.
  /// @param ptr - pointer pointing to the packed weight.
  /// @param size - size of the packed weight.
  size_t (*look_up_or_insert)(void* context, void* ptr, size_t size);
  ...
  enum xnn_status (*finalize)(void* context, enum xnn_weights_cache_finalization_kind finalization_kind);
