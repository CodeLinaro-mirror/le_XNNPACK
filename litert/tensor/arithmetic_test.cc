...
TEST(OperationCastTest, Pad) {
  Tensor a({.type = Type::kFP32, .shape = {2, 4}});
  Tensor b({.type = Type::kI32, .shape = {2, 2}, .buffer = std::vector<int32_t>{0, 0, 0, 0}});
...
TEST(OperationCastTest, PadV2) {
  Tensor a({.type = Type::kFP32, .shape = {2, 4}});
  Tensor b({.type = Type::kI32, .shape = {2, 2}, .buffer = std::vector<int32_t>{0, 0, 0, 0}});
  Tensor c({.type = Type::kFP32, .shape = {}, .buffer = std::vector<float>{0.0f}});
...
TEST(OperationCastTest, Transpose) {
  Tensor a({.type = Type::kFP32, .shape = {2, 4}});
  Tensor b({.type = Type::kI32, .shape = {2}, .buffer = std::vector<int32_t>{1, 0}});
  VerifyOperationRTTI<graph::TransposeOperation>(Transpose(a, b));
}

TEST(OperationCastTest, Tile) {
  Tensor a({.type = Type::kFP32, .shape = {2, 4}});
  Tensor b({.type = Type::kI32, .shape = {2}, .buffer = std::vector<int32_t>{1, 1}});
...
TEST(OperationCastTest, Slice) {
  Tensor a({.type = Type::kFP32, .shape = {2, 4}});
  Tensor begin({.type = Type::kI32, .shape = {2}, .buffer = std::vector<int32_t>{0, 0}});
  Tensor size({.type = Type::kI32, .shape = {2}, .buffer = std::vector<int32_t>{1, 1}});
...
TEST(OperationCastTest, DynamicUpdateSlice) {
  ...
  Tensor c({.type = Type::kI32, .shape = {2}, .buffer = std::vector<int32_t>{0, 0}});
  VerifyOperationRTTI<graph::DynamicUpdateSliceOperation>(DynamicUpdateSlice(a, b, c));
}
...
TEST(OperationCastTest, Quantize) {
  Tensor a({.type = Type::kFP32, .shape = {2, 4}});
  VerifyOperationRTTI<graph::QuantizeOperation>(Quantize(a, Type::kI8, {1.0f}, {0}));
}


TEST(OperationCastTest, Reverse) {
  Tensor a({.type = Type::kFP32, .shape = {2, 4}});
  Tensor b({.type = Type::kI32, .shape = {1}, .buffer = std::vector<int32_t>{0}});
...
TEST(OperationCastTest, NonMaxSuppressionV5) {
  ...
  Tensor score_threshold({.type = Type::kFP32, .shape = {}});
  Tensor soft_nms_sigma({.type = Type::kFP32, .shape = {}});
  VerifyOperationRTTI<graph::NonMaxSuppressionV5Operation>(
      NonMaxSuppressionV5(boxes, scores, 3, iou_threshold, score_threshold, soft_nms_sigma)[0]);
}
