...
struct Reunion {};
template <>
struct DummyTemplate<3, struct enumerator> {};
...
TEST(TypeIdTest, TypeNameExtractionWorks) {
  ...
  EXPECT_THAT((TypeId::Name<DummyTemplate<3, struct enumerator>>()),
              StrEq("litert::tensor::internal::DummyTemplate<3,enumerator>"));
}
