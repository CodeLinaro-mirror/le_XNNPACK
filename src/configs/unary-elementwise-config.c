...
static void init_f32_cosine_config_impl(struct xnn_unary_elementwise_config* config, bool consistent_arithmetic) {
    ...
    (void) hardware_config;  // May be unused.
    if (hardware_config->arch_flags & xnn_arch_arm_neon) {
      config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vcos_ukernel__neon_poly_5_u16);
      config->element_tile = 16;
    } else {
      config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vcos_ukernel__scalar_poly_5_u4);
    ...
    #if XNN_ENABLE_AVX512F
      if (hardware_config->arch_flags & xnn_arch_x86_avx512f) {
        config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vcos_ukernel__avx512f_poly_5_u32);
        config->element_tile = 32;
      } else
    #endif
    #if XNN_ENABLE_FMA3
      if (hardware_config->arch_flags & xnn_arch_x86_fma3) {
        config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vcos_ukernel__fma3_poly_5_u16);
        config->element_tile = 16;
      } else
    #endif
    #if XNN_ENABLE_AVX
  ...
  #elif XNN_ARCH_WASMRELAXEDSIMD
    config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vcos_ukernel__wasmrelaxedsimd_poly_5_u16);
    config->element_tile = 16;
  #elif XNN_ARCH_WASMSIMD
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    ...
    assert(hardware_config != NULL);
    if (hardware_config->arch_flags & xnn_arch_riscv_vector) {
      config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vcos_ukernel__rvv_poly_5_u4v);
      config->element_tile = 4 * hardware_config->vlenb / sizeof(float);
    } else {
      config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vcos_ukernel__scalar_poly_5_u4);
      config->element_tile = 4;
...
static void init_f32_sine_config_impl(struct xnn_unary_elementwise_config* config, bool consistent_arithmetic) {
    ...
    (void) hardware_config;  // May be unused.
    if (hardware_config->arch_flags & xnn_arch_arm_neon) {
      config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vsin_ukernel__neon_poly_5_u16);
      config->element_tile = 16;
    } else {
      config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vsin_ukernel__scalar_poly_5_u4);
      config->element_tile = 4;
    }
  #elif XNN_ARCH_ARM64
    config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vsin_ukernel__neon_poly_5_u16);
    ...
    #if XNN_ENABLE_AVX512F
      if (hardware_config->arch_flags & xnn_arch_x86_avx512f) {
        config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vsin_ukernel__avx512f_poly_5_u32);
        config->element_tile = 32;
      } else
    #endif
    #if XNN_ENABLE_FMA3
      if (hardware_config->arch_flags & xnn_arch_x86_fma3) {
        config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vsin_ukernel__fma3_poly_5_u16);
        config->element_tile = 16;
      } else
    #endif
    #if XNN_ENABLE_AVX
  ...
  #elif XNN_ARCH_WASMRELAXEDSIMD
    config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vsin_ukernel__wasmrelaxedsimd_poly_5_u16);
    config->element_tile = 16;
  #elif XNN_ARCH_WASMSIMD
    const struct xnn_hardware_config* hardware_config = xnn_init_hardware_config();
    assert(hardware_config != NULL);
    ...
    assert(hardware_config != NULL);
    if (hardware_config->arch_flags & xnn_arch_riscv_vector) {
      config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vsin_ukernel__rvv_poly_5_u4v);
      config->element_tile = 4 * hardware_config->vlenb / sizeof(float);
    } else {
      config->ukernel = XNN_INIT_UNARY_UKERNEL(xnn_f32_vsin_ukernel__scalar_poly_5_u4);
      config->element_tile = 4;
