...
void xnn_f16_vcos_ukernel__rvvfp16arith_poly_3_u4v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat16m4_t vx = __riscv_vle16_v_f16m4((const _Float16*)input, vl); input += vl;

    vfloat16m4_t vy = __riscv_vfmul(vx, vtwo_over_pi, vl);
    ...
    vfloat16m4_t vk_quad = __riscv_vfadd(vk, 1.0f16, vl);
    vuint16m4_t vu_quad = __riscv_vreinterpret_u16m4(vk_quad);
    vbool4_t vmask_odd = __riscv_vmseq(__riscv_vand(vu_quad, 1, vl), 1, vl);
    vuint16m4_t vsign_flip = __riscv_vsll(__riscv_vsrl(vu_quad, 1, vl), 15, vl);

    vfloat16m4_t vabs_x = __riscv_vfabs(vx, vl);
    vfloat16m4_t vsub_x = __riscv_vfrsub(vabs_x, vpi_over_two, vl);
    vx = __riscv_vmerge(vx, vsub_x, vmask_odd, vl);
    vx = __riscv_vreinterpret_f16m4(__riscv_vxor(__riscv_vreinterpret_u16m4(vx), vsign_flip, vl));

    const vfloat16m4_t vx2 = __riscv_vfmul(vx, vx, vl);

    vfloat16m4_t vp = __riscv_vfadd(__riscv_vfmul(vx2, vc3, vl), vc2, vl);
    vp = __riscv_vfadd(__riscv_vfmul(vx2, vp, vl), vc1, vl);
    vp = __riscv_vfadd(__riscv_vfmul(vx2, vp, vl), vc0, vl);

    vfloat16m4_t vy_res = __riscv_vfmul(vx, vp, vl);

    __riscv_vse16_v_f16m4((_Float16*)output, vy_res, vl); output += vl;
  } while (batch != 0);
}
