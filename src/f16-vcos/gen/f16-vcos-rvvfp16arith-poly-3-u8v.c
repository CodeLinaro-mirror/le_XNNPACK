...
void xnn_f16_vcos_ukernel__rvvfp16arith_poly_3_u8v(
    size_t batch,
    ...
    const struct xnn_f16_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat16m8_t vy = __riscv_vfmul(vx, vtwo_over_pi, vl);
    ...
    vfloat16m8_t vk_quad = __riscv_vfadd(vk, 1.0f16, vl);
    vuint16m8_t vu_quad = __riscv_vreinterpret_u16m8(vk_quad);
    vbool2_t vmask_odd = __riscv_vmseq(__riscv_vand(vu_quad, 1, vl), 1, vl);
    vuint16m8_t vsign_flip = __riscv_vsll(__riscv_vsrl(vu_quad, 1, vl), 15, vl);

    vfloat16m8_t vabs_x = __riscv_vfabs(vx, vl);
    vfloat16m8_t vsub_x = __riscv_vfrsub(vabs_x, vpi_over_two, vl);
    ...
    vfloat16m8_t vp = __riscv_vfadd(__riscv_vfmul(vx2, vc3, vl), vc2, vl);
