...
class F32GemmLoadsplatGenerator : public WasmAssembler {
  ...
  void generate(const char* name, size_t max_mr, const jit_gemm_params* jit_gemm_params) {
    ...
    AddFunc<10>({}, name, locals_declaration,
                [&](auto mr, auto nc, auto kc, auto a, auto a_stride, auto w, auto c, auto cm_stride, auto cn_stride,
                  ...
                  InitPostOps(post_operations, num_post_operations);
 ...
 private:
  ...
  void InitPostOps(const xnn_post_operation* ops, size_t num_ops) {
    for (size_t i = 0; i < num_ops; i++) {
      switch (ops[i].op_type) {
        case xnn_post_operation_type_hardswish:
          hswish_consts_.vsixth = MakeF32x4Const(1.0 / 6.0);
          hswish_consts_.vsix = MakeF32x4Const(6.0);
          hswish_consts_.vthree = MakeF32x4Const(3.0);
...
xnn_status generate(xnn_code_buffer* b, size_t max_mr, size_t MR, const void* params) {
  assert(max_mr <= MR);
  constexpr static size_t kMaxFuncNameLen = 53;
  char name[kMaxFuncNameLen];
  if (snprintf(name, kMaxFuncNameLen, "xnn_generated_f32_gemm_%zux8__wasmsimd_x86_loadsplat", max_mr) >=
      kMaxFuncNameLen) {
...
extern "C" {
xnn_status_t xnn_generate_f32_gemm_ukernel_1x8__wasmsimd_x86_loadsplat(xnn_code_buffer* b, size_t max_mr,
                                                                       size_t nc_mod_nr, size_t kc,
                                                                       const void* params) {
  return xnnpack::generate(b, max_mr, 1, params);
}

xnn_status_t xnn_generate_f32_gemm_ukernel_3x8__wasmsimd_x86_loadsplat(xnn_code_buffer* b, size_t max_mr,
                                                                       size_t nc_mod_nr, size_t kc,
                                                                       const void* params) {
  return xnnpack::generate(b, max_mr, 3, params);
}

xnn_status_t xnn_generate_f32_gemm_ukernel_4x8__wasmsimd_x86_loadsplat(xnn_code_buffer* b, size_t max_mr,
                                                                       size_t nc_mod_nr, size_t kc,
                                                                       const void* params) {
  return xnnpack::generate(b, max_mr, 4, params);
}

xnn_status_t xnn_generate_f32_gemm_ukernel_5x8__wasmsimd_x86_loadsplat(xnn_code_buffer* b, size_t max_mr,
                                                                       size_t nc_mod_nr, size_t kc,
                                                                       const void* params) {
  return xnnpack::generate(b, max_mr, 5, params);
}

xnn_status_t xnn_generate_f32_gemm_ukernel_6x8__wasmsimd_x86_loadsplat(xnn_code_buffer* b, size_t max_mr,
                                                                       size_t nc_mod_nr, size_t kc,
                                                                       const void* params) {
  return xnnpack::generate(b, max_mr, 6, params);
}
}
