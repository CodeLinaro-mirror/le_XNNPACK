    ...
    const union xnn_f32_default_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 16 * sizeof(float); batch -= 16 * sizeof(float)) {
    ...
    const __m256 va1r = _mm256_loadu_ps(ar + 16);
    const __m256 va1i = _mm256_loadu_ps(ai + 16);
    const __m256 vb1r = _mm256_loadu_ps(br + 16);
    const __m256 vb1i = _mm256_loadu_ps(bi + 16);
    ar += 16;
    ai += 16;
    br += 16;
    bi += 16;
    ...
    _mm256_storeu_ps(or + 16, vacc1r);
    _mm256_storeu_ps(oi + 16, vacc1i);
    or += 16;
    oi += 16;
  }
