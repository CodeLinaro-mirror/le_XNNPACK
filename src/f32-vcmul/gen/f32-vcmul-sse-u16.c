    ...
    const union xnn_f32_default_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 16 * sizeof(float); batch -= 16 * sizeof(float)) {
    ...
    const __m128 va1r = _mm_loadu_ps(ar + 16);
    const __m128 va1i = _mm_loadu_ps(ai + 16);
    const __m128 vb1r = _mm_loadu_ps(br + 16);
    const __m128 vb1i = _mm_loadu_ps(bi + 16);
    const __m128 va2r = _mm_loadu_ps(ar + 32);
    const __m128 va2i = _mm_loadu_ps(ai + 32);
    const __m128 vb2r = _mm_loadu_ps(br + 32);
    const __m128 vb2i = _mm_loadu_ps(bi + 32);
    const __m128 va3r = _mm_loadu_ps(ar + 48);
    const __m128 va3i = _mm_loadu_ps(ai + 48);
    const __m128 vb3r = _mm_loadu_ps(br + 48);
    const __m128 vb3i = _mm_loadu_ps(bi + 48);
    ar += 16;
    ai += 16;
    br += 16;
    bi += 16;
    ...
    _mm_storeu_ps(or + 16, vacc1r);
    _mm_storeu_ps(oi + 16, vacc1i);
    _mm_storeu_ps(or + 32, vacc2r);
    _mm_storeu_ps(oi + 32, vacc2i);
    _mm_storeu_ps(or + 48, vacc3r);
    _mm_storeu_ps(oi + 48, vacc3i);
    or += 16;
    oi += 16;
  }
