...
void xnn_f32_vcos_ukernel__rvv_poly_5_u4v(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* unused_params)
{
  ...
  do {
    ...
    vfloat32m4_t vy = __riscv_vfmul(vx, vtwo_over_pi, vl);
    ...
    vfloat32m4_t vk_quad = __riscv_vfadd(vk, 1.0f, vl);
    vuint32m4_t vu_quad = __riscv_vreinterpret_u32m4(vk_quad);
    vbool8_t vmask_odd = __riscv_vmseq(__riscv_vand(vu_quad, 1, vl), 1, vl);
    vuint32m4_t vsign_flip = __riscv_vsll(__riscv_vsrl(vu_quad, 1, vl), 31, vl);

    vfloat32m4_t vabs_x = __riscv_vfabs(vx, vl);
    vfloat32m4_t vsub_x = __riscv_vfrsub(vabs_x, vpi_over_two, vl);
    ...
    vfloat32m4_t vp = __riscv_vfadd(__riscv_vfmul(vx2, vc5, vl), vc4, vl);
