...
void xnn_f32_vrndd_ukernel__hvx_u128(
    size_t batch,
    const float* input,
    float* output,
    const struct xnn_f32_default_params* restrict params)
{
  ...
  for (; batch >= 128 * sizeof(float); batch -= 128 * sizeof(float)) {
    ...
    const xnn_simd_f32_t vy0 = xnn_floor_f32(vx0);
    const xnn_simd_f32_t vy1 = xnn_floor_f32(vx1);
    const xnn_simd_f32_t vy2 = xnn_floor_f32(vx2);
    const xnn_simd_f32_t vy3 = xnn_floor_f32(vx3);
