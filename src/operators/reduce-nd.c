...
enum xnn_status xnn_create_reduce_nd(
  ...
  switch(datatype) {
    case xnn_datatype_fp16: {
        ...
        sizeof_params = sizeof(param_init);
        break;
      }
  ...
  // Restore unused pointers to NULL.
  ...
  return create_reduce_nd(
    ...
    u32_f32_cvt_config, &params, sizeof(params), sum_op_out);
}
...
static void update_params_mean_qs8(
  xnn_operator_t mean_op,
  size_t num_elements) {
  mean_op->params.qs8_reduce.scalar.scale *= 1.0f / (float) (double) num_elements;
...
static void update_params_mean_qu8(
  xnn_operator_t mean_op,
  size_t num_elements) {
  mean_op->params.qu8_reduce.scalar.scale *= 1.0f / (float) (double) num_elements;
  ...
  pthreadpool_t threadpool)
{
  ...
  void (*update_params)(xnn_operator_t, size_t) = get_update_function(sum_op->type, type);
  switch(type) {
    case xnn_datatype_fp16:
      log2_data_element_size = XNN_LOG2_SIZEOF_HALF;
      log2_accumulator_element_size = XNN_LOG2_SIZEOF_FLOAT;
      scale_params = &sum_op->params.reduce_params.f16_f32acc_scale;
      scale_params_size = sizeof(sum_op->params.reduce_params.f16_f32acc_scale);
      break;
    case xnn_datatype_fp32:
    ...
    case xnn_datatype_qint8:
      ...
      log2_accumulator_element_size = XNN_LOG2_SIZEOF_FLOAT;
      scale_params = &sum_op->params.qs8_reduce;
      scale_params_size = sizeof(sum_op->params.qs8_reduce);
      break;
    case xnn_datatype_quint8:
      log2_data_element_size = XNN_LOG2_SIZEOF_UINT8_T;
      log2_accumulator_element_size = XNN_LOG2_SIZEOF_FLOAT;
      scale_params = &sum_op->params.qu8_reduce;
      scale_params_size = sizeof(sum_op->params.qu8_reduce);
      break;
    default:
  ...
  return reshape_reduce_nd(
      sum_op, num_reduction_axes, reduction_axes, num_input_dims, input_shape,
