  ...
  xnn_operator_t* convert_op_out)
{
  ...
  const struct xnn_prequantize_config* f32_qd8_prequantize_config = xnn_init_f32_qd8_prequantize_config();
  if (f32_qd8_prequantize_config == NULL) {
    xnn_log_error(
        "failed to create %s operator: unsupported hardware configuration",
        xnn_operator_type_to_string(xnn_operator_type_convert_nc_f32_qd8));
    return xnn_status_unsupported_hardware;
  ...
  convert_op->prequantize_config = f32_qd8_prequantize_config;
    ...
    pthreadpool_t threadpool)
{
  ...
  convert_op->context.f32_qd8_convert = (struct f32_qd8_convert_context) {
    ...
    .prequantize_ukernel = convert_op->prequantize_config->ukernel,
    .convert_ukernel = convert_op->convert_config->ukernel,
    .init_params_fn = convert_op->convert_config->init.f32_qs8_cvt,
  };
  ...
  struct xnn_qd8_quantization_params* quantization_params)
{
  ...
  convert_op->context.f32_qd8_convert.quantization_params = quantization_params;
