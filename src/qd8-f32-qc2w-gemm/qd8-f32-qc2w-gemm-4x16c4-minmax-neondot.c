...
void xnn_qd8_f32_qc2w_gemm_minmax_ukernel_4x16c4__neondot(
    ...
    const struct xnn_qd8_qc2w_quantization_params*
        quantization_params) XNN_OOB_READS {
  assert(mr != 0);
  assert(mr <= 4);
  assert(nc != 0);
  assert(kc != 0);
  ...
  do {
    ...
    const float32x4_t lh_row_sum_0 =
        vld1q_dup_f32(&quantization_params[0].row_sum);
    const float32x4_t lh_row_sum_1 =
        vld1q_dup_f32(&quantization_params[1].row_sum);
    const float32x4_t lh_row_sum_2 =
        vld1q_dup_f32(&quantization_params[2].row_sum);
    const float32x4_t lh_row_sum_3 =
        vld1q_dup_f32(&quantization_params[3].row_sum);
