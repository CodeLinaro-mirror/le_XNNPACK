    ...
    const struct xnn_qd8_quantization_params quantization_params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  do {
    ...
    __m128i vacc2x0 = _mm_cvtsi32_si128(((const int*) w)[0] * quantization_params[2].zero_point);
    __m128i vacc2x1 = _mm_cvtsi32_si128(((const int*) w)[1] * quantization_params[2].zero_point);
    __m128i vacc2x2 = _mm_cvtsi32_si128(((const int*) w)[2] * quantization_params[2].zero_point);
    __m128i vacc2x3 = _mm_cvtsi32_si128(((const int*) w)[3] * quantization_params[2].zero_point);
    w = (const int32_t*) w + 4;
    ...
    const __m128 vscale2 = _mm_set1_ps(quantization_params[2].scale);
    ...
    vout2x0123 = _mm_add_ps(vout2x0123, vbias);
    ...
    if XNN_LIKELY(nc >= 4) {
      ...
      if (nc & 2) {
        _mm_storel_epi64((__m128i*) c0, vout0x0123);
        _mm_storel_epi64((__m128i*) c1, vout1x0123);
        _mm_storel_epi64((__m128i*) c2, vout2x0123);
        vout0x0123 = _mm_unpackhi_epi64(vout0x0123, vout0x0123);
        c0 += 2;
        vout1x0123 = _mm_unpackhi_epi64(vout1x0123, vout1x0123);
        c1 += 2;
        vout2x0123 = _mm_unpackhi_epi64(vout2x0123, vout2x0123);
        c2 += 2;
      }
      if (nc & 1) {
        unaligned_store_u32(c0, (uint32_t) _mm_cvtsi128_si32(vout0x0123));
        unaligned_store_u32(c1, (uint32_t) _mm_cvtsi128_si32(vout1x0123));
        unaligned_store_u32(c2, (uint32_t) _mm_cvtsi128_si32(vout2x0123));
      }
      nc = 0;
    }
