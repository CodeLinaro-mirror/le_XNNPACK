...
void xnn_qs8_packw_gemm_gio_ukernel_x96c4__scalar(
  size_t g,
  size_t nc,
  size_t kc,
  size_t nr,
  ...
  do {
    ...
    for (;n >= 96; n -= 96) {
      ...
      if XNN_LIKELY(b != NULL) {
        memcpy(out, b, 96 * sizeof(int32_t));
        b += 96;
      } else {
        memset(out, 0, 96 * sizeof(int32_t));
      }
