...
//
neon.// Copyright 2024 Google LLC
//
    ...
    const union xnn_qs8_rsum_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  for (; batch >= 8192; batch -= 8192) {
    ...
    for (size_t current_batch = 8192; current_batch > 0; current_batch -= 128) {
      ...
      vacc16_1 = _mm256_add_epi16(vacc16_1, vt3);
    }
  ...
  if (XNN_UNLIKELY(batch != 0)) {
    ...
    if (XNN_UNLIKELY(batch != 0)) {
      ...
      const __m128i vonemask = _mm_loadu_si128((const __m128i*) &params->ssse3.onemask_table[32 - batch]);
      const __m128i vt = _mm_maddubs_epi16(vonemask, _mm_loadu_si128((const __m128i*) input));
      vacc16 = _mm_add_epi16(vacc16, _mm256_castsi128_si256(vt));
    }
    vacc0 = _mm256_add_epi32(vacc0, _mm256_madd_epi16(vone_16, vacc16));
  }
  vacc0 = _mm256_add_epi32(vacc0, vacc1);

  int32_t res = _mm256_reduce_add_epi32(vacc0);
  *output += res;
}
