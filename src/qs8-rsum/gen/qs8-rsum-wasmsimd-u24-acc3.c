...
void xnn_qs8_rsum_ukernel__wasmsimd_u24_acc3(
    size_t batch,
    const int8_t* input,
    int32_t* output,
    const struct xnn_qs8_rsum_params params[restrict XNN_MIN_ELEMENTS(1)]) XNN_OOB_READS
{
  ...
  v128_t vacc2 = wasm_i32x4_const_splat(0);

  // 256 int8s may be summed into an int16 before overflowing.
  // Each register has 8 lanes and there are 3 accumulators so batch size is 6144
  for (; batch >= 6144; batch -= 6144) {
    v128_t vacc16_0 = wasm_i16x8_const_splat(0);
    v128_t vacc16_1 = wasm_i16x8_const_splat(0);
    v128_t vacc16_2 = wasm_i16x8_const_splat(0);
    for (size_t current_batch = 6144; current_batch > 0; current_batch -= 24) {
      ...
      vacc16_2 = wasm_i16x8_add(vacc16_2, vt2);
    }
    ...
    vacc2 = wasm_i32x4_add(vacc2, wasm_i32x4_extend_low_i16x8(vacc16_2));
    vacc2 = wasm_i32x4_add(vacc2, wasm_i32x4_extend_high_i16x8(vacc16_2));
  }

  if (XNN_UNLIKELY(batch != 0)) {
    assert(batch >= 1 && batch < 6144);
    v128_t vacc16_0 = wasm_i8x16_const_splat(0);
    v128_t vacc16_1 = wasm_i8x16_const_splat(0);
    v128_t vacc16_2 = wasm_i8x16_const_splat(0);
  ...
  vacc0 = wasm_i32x4_add(vacc0, vacc2);
