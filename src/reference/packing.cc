        ...
        for (size_t kr_block_start = 0; kr_block_start < round_up_po2(kc, skr);
          ...
          for (size_t nr_block_offset = 0; nr_block_offset < nr_block_size;
            ...
            for (size_t kr_block_offset = 0; kr_block_offset < kr;
                 kr_block_offset++) {
              const size_t kc_idx = kc_begin + kr_block_offset;
              packed_weights[kr_block_offset] = kc_idx < kc ?
                  k[kc_idx * k_stride + nr_block_start + nr_block_offset] : UINT16_C(0);
            }
            packed_weights += kr;
          }
          packed_weights += (nr - nr_block_size) * kr;
      ...
      for (size_t kr_block_start = 0; kr_block_start < round_up_po2(kc, skr);
        ...
        for (size_t nr_block_offset = 0; nr_block_offset < nr_block_size;
          ...
          for (size_t kr_block_offset = 0; kr_block_offset < kr;
            ...
            const size_t kc_idx = kc_begin + kr_block_offset;
            packed_weights[kr_block_offset] =
                kc_idx < kc ? k[kc_idx * k_stride + nr_block_start + nr_block_offset] : 0.0f;
          }
          packed_weights += kr;
        }
        packed_weights += (nr - nr_block_size) * kr;
      ...
      for (size_t kr_block_start = 0; kr_block_start < round_up_po2(kc, skr);
        ...
        for (size_t nr_block_offset = 0; nr_block_offset < nr_block_size;
          ...
          for (size_t kr_block_offset = 0; kr_block_offset < kr;
               kr_block_offset++) {
            const size_t kc_idx = kc_begin + kr_block_offset;
            const int8_t kv = kc_idx < kc ?
                k[kc_idx * k_stride + (nr_block_start + nr_block_offset)] : INT8_C(0);
            ksum += (uint32_t)kv;
            ((int8_t*)packed_weights)[kr_block_offset] = kv;
          }
          packed_b[nr_block_offset] = packed_b[nr_block_offset] - ksum * izp;
      ...
      for (size_t kr_block_start = 0; kr_block_start < round_up_po2(kc, skr);
        ...
        for (size_t nr_block_offset = 0; nr_block_offset < nr_block_size;
          ...
          for (size_t kr_block_offset = 0; kr_block_offset < kr;
            ...
            const size_t kc_idx = kc_begin + kr_block_offset;
            const int8_t kv =
                kc_idx < kc ? k[kc_idx * k_stride + (nr_block_start + nr_block_offset)] : INT8_C(0);
            ((int8_t*)packed_weights)[kr_block_offset] = kv;
          }
          packed_weights = (int8_t*)packed_weights + kr;
        }
            ...
            for (size_t kx = ox; kx < kw; kx += sw) {
              for (size_t kr_block_start = 0;
                ...
                for (size_t nr_block_offset = 0;
                  ...
                  std::fill((int8_t*)packed_weights, end,
                            INT8_C(0));
                  packed_weights = end;
                }
                packed_weights =
  ...
  // Pack in blocks of channel_tile, then in blocks of channel_subtile.
  {
    ...
    for (; cr_block_start < tiled_c; cr_block_start += channel_tile) {
      ...
      std::fill_n(packed_weights, (primary_tile - kernel_size) * channel_tile,
                  0.0f);
      packed_weights += (primary_tile - kernel_size) * channel_tile;
    }
    ...
    for (; cr_block_start < c; cr_block_start += channel_subtile) {
      ...
      std::fill_n(packed_weights,
                  (primary_tile - kernel_size) * channel_subtile, 0.0f);
      packed_weights += (primary_tile - kernel_size) * channel_subtile;
    }
  }
}
...
void xnn_pack_f16_dwconv_ghw_w(
  ...
  // Pack in blocks of channel_tile, then in blocks of channel_subtile.
  {
    ...
    for (; cr_block_start < c; cr_block_start += channel_subtile) {
      ...
      std::fill_n(packed_weights,
                  (primary_tile - kernel_size) * channel_subtile,
                  static_cast<xnn_float16>(0.0f));
      packed_weights += (primary_tile - kernel_size) * channel_subtile;
    }
  }
...
void xnn_pack_qs8_dwconv_ghw_w(size_t primary_tile, size_t h, size_t w,
  ...
  // Pack in blocks of channel_tile, then in blocks of channel_subtile.
  {
    ...
    for (; cr_block_start < tiled_c; cr_block_start += channel_tile) {
      unaligned_int32_t* packed_b = (unaligned_int32_t*)packed_weights;
      const size_t cr_block_size = min(c - cr_block_start, channel_tile);
      std::fill_n((int8_t*)packed_weights, channel_tile * sizeof(int32_t),
                  INT8_C(0));
    ...
    for (; cr_block_start < c; cr_block_start += channel_subtile) {
      unaligned_int32_t* packed_b = (unaligned_int32_t*)packed_weights;
      const size_t cr_block_size = min(c - cr_block_start, channel_subtile);
      std::fill_n((int8_t*)packed_weights, channel_subtile * sizeof(int32_t),
                  INT8_C(0));
      copy_bias(b, cr_block_start, cr_block_size, packed_b);
      packed_weights = (void*)((uintptr_t)packed_weights +
                               channel_subtile * sizeof(int32_t));
      ...
      for (size_t i = 0; i < kernel_size; i++) {
        ...
        std::fill_n((int8_t*)packed_weights, channel_subtile - cr_block_size,
                    INT8_C(0));
        packed_weights =
            (void*)((uintptr_t)packed_weights +
...
void xnn_pack_f16_dwconv_hwg_w(
  ...
  // Pack in blocks of channel_tile, then in blocks of channel_subtile.
  {
    ...
    for (; cr_block_start < c; cr_block_start += channel_subtile) {
      ...
      std::fill_n(packed_weights,
                  (primary_tile - kernel_size) * channel_subtile, UINT16_C(0));
      packed_weights += (primary_tile - kernel_size) * channel_subtile;
    }
  }
}
