      ...
      for (size_t kr_block_start = 0;
        ...
        for (size_t nr_block_offset = 0; nr_block_offset < nr_block_size;
          ...
          for (size_t kr_block_offset = 0; kr_block_offset < kr;
            ...
            const size_t k1_offset = k_offset + kr;
            if (kc_idx + kr < kc) {
              kv_1 = (k[k1_offset >> 2] >> ((k1_offset & 3) * 2)) & 0x3;
            }
            const size_t k2_offset = k_offset + 2 * kr;
            if (kc_idx + 2 * kr < kc) {
              kv_2 = (k[k2_offset >> 2] >> ((k2_offset & 3) * 2)) & 0x3;
            }
            const size_t k3_offset = k_offset + 3 * kr;
            if (kc_idx + 3 * kr < kc) {
              kv_3 = (k[k3_offset >> 2] >> ((k3_offset & 3) * 2)) & 0x3;
            }
