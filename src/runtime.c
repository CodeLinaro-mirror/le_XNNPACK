...
enum xnn_status xnn_create_weights_cache_with_size(size_t size, xnn_weights_cache_t* weights_cache_out)
{
  ...
  cache_provider->release = xnn_internal_release_weights_cache;
  cache_provider->look_up = xnn_internal_weights_cache_look_up;
  cache_provider->reserve_space = xnn_internal_reserve_space_in_weights_cache;
  cache_provider->look_up_or_insert = xnn_internal_get_or_insert_weights_cache;
  cache_provider->finalize = xnn_internal_finalize_weights_cache;
  cache_provider->is_finalized = xnn_internal_weights_cache_is_finalized;
  cache_provider->offset_to_addr = xnn_internal_weights_cache_offset_to_addr;
  cache_provider->delete_cache = xnn_internal_delete_weights_cache;
...
enum xnn_status xnn_delete_weights_cache(xnn_weights_cache_t weights_cache)
{
  if XNN_LIKELY(weights_cache != NULL) {
    enum xnn_status status = weights_cache->release(weights_cache->context);
    if (status != xnn_status_success) {
      return status;
    }
