...
static bool input_memory_can_be_reused(const xnn_subgraph_t subgraph, const xnn_runtime_t runtime, size_t input_id, size_t output_id)
{
  ...
  const xnn_runtime_t runtime)
{
  ...
  for (uint32_t n = 0; n < subgraph->num_nodes; n++) {
    ...
    for (size_t i = 0; i < node->num_inputs; i++) {
      if (input_memory_can_be_reused(subgraph, runtime, node->inputs[i], node->outputs[0])) {
  ...
  xnn_runtime_t* runtime_out)
{
  ...
  if (runtime->values == NULL) {
    xnn_log_error("failed to allocate %zu bytes for value descriptors",
  ...
  memcpy(runtime->values, subgraph->values, sizeof(struct xnn_value) * subgraph->num_values);
  ...
  for (uint32_t i = 0; i < runtime->num_values; i++) {
    ...
    if (value->data == NULL) {
      ...
      // Runtime takes ownership of the data.
      value->data = NULL;
    }
  }
