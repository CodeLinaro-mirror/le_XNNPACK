  ...
  const struct xnn_value* src_value)
{
  ...
  if (src_value->fp16_compatible == 1) {
    ...
    xnn_subgraph_t subgraph)
{
  ...
  for (uint32_t i = 0; i < subgraph->num_values; i++) {
    ...
    if (value->num_consumers == 1) {
      ...
      if (consumer->type == xnn_node_type_copy && xnn_value_is_valid(value->type) && xnn_value_is_internal(value) &&
      ...
      if (producer->type == xnn_node_type_copy && xnn_value_is_valid(value->type) && xnn_value_is_internal(value) &&
  ...
  uint32_t optimization_flags)
{
  ...
  for (uint32_t i = 0; i < subgraph->num_values; i++) {
    ...
    if (!xnn_value_is_external_input(value->flags) && value->num_consumers == 0 && !xnn_value_is_persistent(value->flags, value->allocation_type)) {
