  ...
  xnn_weights_cache_t weights_cache)
{
  ...
  for (size_t i = 0; i < num_splits; ++i) {
    assert(opdata->outputs[i] != XNN_INVALID_VALUE_ID);
    assert(opdata->outputs[i] < num_values);
    if (values[opdata->outputs[i]].type != xnn_value_type_invalid) {
      datatype = values[opdata->outputs[i]].datatype;
    }
  }
