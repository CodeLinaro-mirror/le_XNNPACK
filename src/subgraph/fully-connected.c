...
enum fully_connected_op_type {
  ...
  fc_type_f16_f32_f16,
  fc_type_qd8_f16_qc2w,
  fc_type_qdu8_f16_qc2w,
  ...
  fc_type_qs8_qs8_qc2w,
  fc_type_qs8_qs8_qc4w,
};
        ...
        case xnn_datatype_qcint2:
          switch (input_datatype) {
            case xnn_datatype_qdint8:
              return fc_type_qd8_f16_qc2w;
            case xnn_datatype_qduint8:
              return fc_type_qdu8_f16_qc2w;
            default:
              XNN_UNREACHABLE;
          }
          break;
...
static enum xnn_status create_fully_connected_operator(
  ...
  switch (op_type) {
    ...
    case fc_type_qd8_f16_qc2w:
      status = xnn_create_fully_connected_nc_qd8_f16_qc2w(
          input_channels, output_channels,
          /*input_stride=*/input_channels,
          /*output_stride=*/output_channels,
          ...
          weights_cache, fully_connected_op_ptr);
      break;
    case fc_type_qdu8_f16_qc2w:
      status = xnn_create_fully_connected_nc_qdu8_f16_qc2w(
          input_channels, output_channels,
          /*input_stride=*/input_channels,
          /*output_stride=*/output_channels,
          filter_value->quantization.channelwise_zero_point,
    ...
    case fc_type_qdu8_f32_qc8w:
      status = xnn_create_fully_connected_nc_qdu8_f32_qc8w(
          ...
          weights_cache, fully_connected_op_ptr);
      break;
    case fc_type_qs8_qs8_qc2w: {
      assert(!has_non_static_weights);
      assert(kernel_data != NULL);
      assert(filter_value->datatype == xnn_datatype_qcint2);
      assert(filter_value->quantization.zero_point == 0);
      ...
      status = xnn_create_fully_connected_nc_qs8_qc2w(
          input_channels, output_channels,
          /*input_stride=*/input_channels,
          /*output_stride=*/output_channels,
...
static enum xnn_status reshape_fully_connected_operator(
  ...
  switch (fully_connected_op->type) {
    ...
    case xnn_operator_type_fully_connected_nc_qd8_f16_qc2w:
      status = xnn_reshape_fully_connected_nc_qd8_f16_qc2w(
          fully_connected_op, batch_size, &opdata->workspace_size, threadpool);
      break;
    case xnn_operator_type_fully_connected_nc_qdu8_f16_qc2w:
      status = xnn_reshape_fully_connected_nc_qdu8_f16_qc2w(
          fully_connected_op, batch_size, &opdata->workspace_size, threadpool);
      break;
    case xnn_operator_type_fully_connected_nc_qd8_f32_qc2w:
      status = xnn_reshape_fully_connected_nc_qd8_f32_qc2w(
          fully_connected_op, batch_size, &opdata->workspace_size, threadpool);
      break;
    case xnn_operator_type_fully_connected_nc_qdu8_f32_qc2w:
      status = xnn_reshape_fully_connected_nc_qdu8_f32_qc2w(
          fully_connected_op, batch_size, &opdata->workspace_size, threadpool);
      break;
    ...
    case xnn_operator_type_fully_connected_nc_qs8_qc2w:
      status = xnn_reshape_fully_connected_nc_qs8_qc2w(fully_connected_op,
                                                       batch_size, threadpool);
      break;
...
static enum xnn_status setup_fully_connected_operator(
  ...
  switch (fully_connected_op->type) {
    ...
    case xnn_operator_type_fully_connected_nc_qd8_f16_qc2w: {
      ...
      float* row_sum = input_value->quantization.row_sum;
      void* workspace = opdata->workspace;
      assert(kernel_data == NULL);
      assert(bias_data == NULL);
      assert(quantization_params != NULL || workspace != NULL);
      return xnn_setup_fully_connected_nc_qd8_f16_qc2w(
          fully_connected_op, input_data, output_data, workspace, row_sum,
          quantization_params);
    }
    case xnn_operator_type_fully_connected_nc_qdu8_f16_qc2w: {
      const void* quantization_params =
      ...
      assert(bias_data == NULL);
      assert(quantization_params != NULL || workspace != NULL);
      return xnn_setup_fully_connected_nc_qdu8_f16_qc2w(
          fully_connected_op, input_data, output_data, workspace, row_sum,
          quantization_params);
    }
    case xnn_operator_type_fully_connected_nc_qd8_f32_qc2w: {
      ...
      assert(bias_data == NULL);
      assert(quantization_params != NULL || workspace != NULL);
      return xnn_setup_fully_connected_nc_qd8_f32_qc2w(
          fully_connected_op, input_data, output_data, workspace, row_sum,
          quantization_params);
    }
    case xnn_operator_type_fully_connected_nc_qdu8_f32_qc2w: {
      ...
      assert(bias_data == NULL);
      assert(quantization_params != NULL || workspace != NULL);
      return xnn_setup_fully_connected_nc_qdu8_f32_qc2w(
          fully_connected_op, input_data, output_data, workspace, row_sum,
          quantization_params);
    }
    case xnn_operator_type_fully_connected_nc_qd8_f32_qc4w: {
    ...
    case xnn_operator_type_fully_connected_nc_qs8_qc2w:
      assert(kernel_data == NULL);
      assert(bias_data == NULL);
      return xnn_setup_fully_connected_nc_qs8_qc2w(fully_connected_op,
                                                   input_data, output_data);
    case xnn_operator_type_fully_connected_nc_qs8_qc4w:
      assert(kernel_data == NULL);
      assert(bias_data == NULL);
    ...
    case xnn_datatype_qcint2:
      if (input_datatype == xnn_datatype_qdint8 &&
        ...
        return true;
                 ...
                 (bias_datatype == xnn_datatype_qcint32 || bias_datatype == xnn_datatype_qint32) &&
...
enum xnn_status xnn_define_fully_connected(xnn_subgraph_t subgraph,
    ...
    case xnn_datatype_qcint2:
      if (kernel_value->quantization.zero_point != 0) {
        xnn_log_error("failed to define %s operator with filter ID #%" PRIu32
                      ...
                      " for datatype %s, must be equals to 0 ",
                      xnn_node_type_to_string(xnn_node_type_fully_connected),
  ...
  const bool is_channelwise_quantized =
      ...
      kernel_value->datatype == xnn_datatype_qcint2;
