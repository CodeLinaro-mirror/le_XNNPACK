  ...
  pthreadpool_t threadpool)
{
  ...
  size_t old_dynamic_quant_params_size = xnn_runtime_tensor_get_dynamic_quant_param_size(output);
  memcpy(&output->shape.dim[0], &input->shape.dim[0], input->shape.num_dims * sizeof(size_t));
  output->quantization.dynamic_params_size = xnn_runtime_tensor_get_dynamic_quant_param_size(output);
