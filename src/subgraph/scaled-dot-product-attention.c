  ...
  pthreadpool_t threadpool)
{
  ...
  if (query_num_dims < 4) {
    xnn_log_error(
      "failed to reshape %s operator with query ID #%" PRIu32 ": query must have at least 4 dimensions",
  ...
  if (key_num_dims < 3) {
    xnn_log_error(
      "failed to reshape %s operator with key ID #%" PRIu32 ": key must have at least 3 dimensions",
  ...
  uint32_t flags)
{
  ...
  // Query must have at least 4 dimensions.
  const size_t query_num_dims = query->shape.num_dims;
  if (query_num_dims < 4) {
    xnn_log_error(
      "failed to define %s operator with query ID #%" PRIu32 ": query must have at least 4 dimensions",
  ...
  // Key must have at least 3 dimensions.
  const size_t key_num_dims = key->shape.num_dims;
  if (key_num_dims < 3) {
    xnn_log_error(
      "failed to define %s operator with key ID #%" PRIu32 ": key must have at least 3 dimensions",
  ...
  // Value must have at least 3 dimensions.
  const size_t value_num_dims = value->shape.num_dims;
  if (value_num_dims < 3) {
    xnn_log_error(
      "failed to define %s operator with value ID #%" PRIu32 ": value must have at least 3 dimensions",
  ...
  // Output must have at least 4 dimensions.
  const size_t output_num_dims = output->shape.num_dims;
  if (output_num_dims < 4) {
    xnn_log_error(
      "failed to define %s operator with output ID #%" PRIu32 ": output must have at least 4 dimensions",
