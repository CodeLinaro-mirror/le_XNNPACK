...
enum xnn_status xnn_define_blockwise_quantized_tensor_value_v2(
    ...
    uint32_t external_id,
    uint32_t flags,
    enum xnn_scale_type scale_type,
    uint32_t* id_out)
{
  if ((xnn_params.init_flags & XNN_INIT_FLAG_XNNPACK) == 0) {
  ...
  switch (scale_type) {
    case xnn_scale_type_bf16:
    case xnn_scale_type_fp16:
      break;
    default:
      xnn_log_error("failed to create Blockwise Quantized Dense Tensor value: unsupported scale_type");
      return xnn_status_unsupported_parameter;
  }
  ...
  for (size_t block = 0; block < block_count; block++) {
    ...
    switch (scale_type) {
      case xnn_scale_type_bf16:
        float_scale = math_cvt_fp32_bf16(scale[block]);
        break;
      case xnn_scale_type_fp16:
  ...
  switch (scale_type) {
    case xnn_scale_type_bf16:
      ...
      value->quantization.scale_type = xnn_scale_type_bf16;
      break;
    case xnn_scale_type_fp16:
      value->quantization.blockwise_scale.fp16_scale = (const xnn_float16*) scale;
      value->quantization.scale_type = xnn_scale_type_fp16;
      break;
    default:
      XNN_UNREACHABLE;
  }
...
enum xnn_status xnn_define_blockwise_quantized_tensor_value(
  ...
  return xnn_define_blockwise_quantized_tensor_value_v2(subgraph, datatype, zero_point, scale, num_dims, channel_dim,
                                                        block_size, dims, data, external_id, flags, xnn_scale_type_bf16, id_out);
}
