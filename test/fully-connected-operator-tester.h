...
class FullyConnectedOperatorTester {
 public:
  ...
  void TestQD8F32QC8W() const {
    ...
    std::vector<float> input_float(batch_size() * input_channels());
    ...
    std::vector<xnn_qd8_quantization_params> quantization_params(4 + batch_size());
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      for (size_t m_index = 0; m_index < batch_size(); m_index++) {
        const float* input_ptr = &input_float[m_index * input_channels()];
        const auto minmax = std::minmax_element(input_ptr, input_ptr + input_channels());
        quantization_params[m_index] = xnn_f32_qd8_asymmetric_quantization_params(*minmax.first, *minmax.second);
        for (size_t k_index = 0; k_index < input_channels(); k_index++) {
          float scaled_input = input_ptr[k_index] / quantization_params[m_index].inv_scale;
          ...
          scaled_input = std::max<float>(scaled_input, float(std::numeric_limits<int8_t>::min()
                                                             - quantization_params[m_index].zero_point));
          input[m_index * input_stride() + k_index] = int8_t(std::lrintf(scaled_input) + long(quantization_params[m_index].zero_point));
        }
      }
      if (transpose_weights()) {
        for (size_t m_index = 0; m_index < batch_size(); m_index++) {
          for (size_t n_index = 0; n_index < output_channels(); n_index++) {
            int32_t ksum = 0;
            for (size_t k_index = 0; k_index < input_channels(); k_index++) {
              ksum += kernel[k_index * output_channels() + n_index];
            ...
            output_ref[m_index * output_channels() + n_index] -= (quantization_params[m_index].zero_point * ksum);
        ...
        for (size_t m_index = 0; m_index < batch_size(); m_index++) {
          for (size_t n_index = 0; n_index < output_channels(); n_index++) {
            int32_t ksum = 0;
            for (size_t k_index = 0; k_index < input_channels(); k_index++) {
              ksum += kernel[n_index * input_channels() + k_index];
              output_ref[m_index * output_channels() + n_index] +=
                  int32_t(input[m_index * input_stride() + k_index]) *
      ...
      ASSERT_EQ(xnn_status_success,
        xnn_setup_fully_connected_nc_qd8_f32_qc8w(
          ...
          (const struct xnn_dynamic_quantization_params*) quantization_params.data()));
