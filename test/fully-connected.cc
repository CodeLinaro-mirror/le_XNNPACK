...
TEST_F(FullyConnectedTestF32QC4W, define)
{
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_channelwise_quantized_tensor_value_v2(
                          subgraph, xnn_datatype_qcint4, /*zero_point=*/0, requantization_scales.data(),
...
TEST_F(FullyConnectedTestQD8F32QC8W, define)
{
  ...
  ASSERT_EQ(
    xnn_status_success, xnn_define_dynamically_quantized_tensor_value(
                          subgraph, xnn_datatype_qdint8, /*non_batch_dims=*/1, input_dims.size(), input_dims.data(),
                          /*external_id=*/0, /*flags=*/0, &input_id));
  ASSERT_NE(input_id, XNN_INVALID_VALUE_ID);
...
TEST_F(FullyConnectedTestQD8F32QC8W, matches_operator_api)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));

  xnn_operator_t op = nullptr;

  std::vector<float> input_float(NumElements(input_dims) + XNN_EXTRA_BYTES / sizeof(float));
  ...
  for (int i = 0; i < batch_size; ++i) {
    const float* input_ptr = &input_float[i * input_channels];
    const auto minmax = std::minmax_element(input_ptr, input_ptr + input_channels);
    ...
    for (int j = 0; j < input_channels; ++j) {
      ...
      scaled_input = std::max<float>(scaled_input, float(std::numeric_limits<int8_t>::min()
                                                         - quantization_params[i].zero_point));
      input[i * input_channels + j] = int8_t(std::lrintf(scaled_input) + long(quantization_params[i].zero_point));
    }
  }
  std::generate(kernel.begin(), kernel.end(), [&]() { return w8dist(rng); });
  std::generate(bias.begin(), bias.end(), [&]() { return f32dist(rng); });
  ...
  ASSERT_EQ(xnn_status_success, status);
  ASSERT_NE(nullptr, op);
  ASSERT_EQ(xnn_status_success, xnn_reshape_fully_connected_nc_qd8_f32_qc8w(op, batch_size, /*threadpool=*/nullptr));
  ASSERT_EQ(xnn_status_success, xnn_setup_fully_connected_nc_qd8_f32_qc8w(op, input.data(), operator_output.data(), (xnn_dynamic_quantization_params*)quantization_params.data()));
  ...
  ASSERT_NE(nullptr, runtime);
  std::unique_ptr<xnn_runtime, decltype(&xnn_delete_runtime)> auto_runtime(runtime, xnn_delete_runtime);
  std::array<xnn_external_value, 2> external = {xnn_external_value{input_id, input.data(), quantization_params.data()}, xnn_external_value{output_id, subgraph_output.data()}};
  ASSERT_EQ(xnn_status_success, xnn_setup_runtime(runtime, external.size(), external.data()));
  ASSERT_EQ(xnn_status_success, xnn_invoke_runtime(runtime));
...
TEST_F(FullyConnectedTestQD8F32QC8W, matches_operator_api_without_bias)
{
  ASSERT_EQ(xnn_status_success, xnn_initialize(/*allocator=*/nullptr));

  xnn_operator_t op = nullptr;

  std::vector<float> input_float(NumElements(input_dims));
  std::generate(input_float.begin(), input_float.end(), [&]() { return f32dist(rng); });
  std::vector<xnn_qd8_quantization_params> quantization_params(batch_size + 10);
  for (int i = 0; i < batch_size; ++i) {
    const float* input_ptr = &input_float[i * input_channels];
    const auto minmax = std::minmax_element(input_ptr, input_ptr + input_channels);
    quantization_params[i] = xnn_f32_qd8_asymmetric_quantization_params(*minmax.first, *minmax.second);
...
TEST_F(FullyConnectedTestQD8F32QC8W, internally_allocated_dynamic_quantization_parameters)
{
  ...
  ASSERT_EQ(xnn_status_success, xnn_setup_convert_nc_f32_qd8(convert_op, convert_input.data(), operator_dq_data.data(), quantization_params.data()));
  ...
  ASSERT_EQ(xnn_status_success, xnn_setup_fully_connected_nc_qd8_f32_qc8w(fc_op, operator_dq_data.data(), operator_output.data(), quantization_params.data()));
