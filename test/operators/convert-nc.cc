...
class ConvertOperatorTester {
 public:
  ...
  int16_t zero_point() const { return this->zero_point_; }
  ...
  void TestF16toQD8() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      std::copy(input.begin(), input.begin() + channels(), input_float.begin());
  ...
  void TestF32toQD8() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_convert_nc_f32_qd8(
                                        /*flags=*/0, &convert_op));
      ASSERT_NE(nullptr, convert_op);
  ...
  void TestF32toQD8QC2W() const {
    ...
    for (size_t iteration = 0; iteration < iterations(); iteration++) {
      ...
      ASSERT_EQ(xnn_status_success, xnn_create_convert_nc_f32_qd8_qc2w(
                                        /*flags=*/0, &convert_op));
      ASSERT_NE(nullptr, convert_op);
      ...
      ASSERT_EQ(xnn_status_success,
                xnn_reshape_convert_nc_f32_qd8_qc2w(
                    convert_op, batch_size(), channels(), input_stride(),
                    output_stride(), /*threadpool=*/nullptr));
      ASSERT_EQ(xnn_status_success, xnn_setup_convert_nc_f32_qd8_qc2w(
                                        convert_op, input.data(), output.data(),
                                        quantization_params.data()));
      ASSERT_EQ(xnn_status_success,
                xnn_run_operator(convert_op, /*threadpool=*/nullptr));
