...
class F32SimdHVXTest : public ::testing::Test {
 protected:
  void SetUp() override {
    TEST_REQUIRES_HVX;
...
TEST_F(F32SimdHVXTest, ReduceAdd) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdHVXTest, ShiftRight) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  // Not using a loop since the `bits` parameter needs to be a compile-time
  // constant, e.g. for `neon`.
  {
...
TEST_F(F32SimdHVXTest, ShiftRightSigned) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  // Not using a loop since the `bits` parameter needs to be a compile-time
  // constant, e.g. for `neon`.
  {
...
TEST_F(F32SimdHVXTest, Reduce_Add) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdHVXTest, StoreTail) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
