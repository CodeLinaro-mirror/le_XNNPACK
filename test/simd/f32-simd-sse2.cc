...
TEST_F(F32SimdSSE2Test, ShiftLeft) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdSSE2Test, ShiftRight) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  // Not using a loop since the `bits` parameter needs to be a compile-time
  // constant, e.g. for `neon`.
  {
...
TEST_F(F32SimdSSE2Test, ShiftRightSigned) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
  // Not using a loop since the `bits` parameter needs to be a compile-time
  // constant, e.g. for `neon`.
  {
...
TEST_F(F32SimdSSE2Test, Reduce_Add) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
...
TEST_F(F32SimdSSE2Test, StoreTail) {
  const xnn_simd_f32_t a = xnn_loadu_f32(inputs_.data());
