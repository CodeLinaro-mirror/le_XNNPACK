...
Tensor<float> ReferenceImpl(Tensor<Data> input, Tensor<Filter> filter,
  ...
  padded = padded.split(3, group_input_channels);
  output = output.split(3, group_output_channels);
  if (!bias.empty()) {
    bias = bias.split(0, group_output_channels);
  }
  filter = filter.split(0, group_output_channels);
  if (!filter_scale.empty()) {
    filter_scale = filter_scale.split(0, group_output_channels);
  }
    ...
    for (int reshape = 0; reshape < 2; ++reshape) {
      ...
      if (convert_to != xnn_datatype_invalid) {
        ...
        for (size_t b = 0; b < input.extent(0); ++b) {
          if (kw.size == 1 && kh.size == 1) {
            // 1x1 conv gets rewritten to fully connected, which dynamically
            // quantizes at a finer granularity.
            for (size_t y = 0; y < input.extent(1); ++y) {
              for (size_t x = 0; x < input.extent(2); ++x) {
                FakeDynamicQuantize(input.slice_leading({b, y, x}), convert_to);
              }
            }
          } else {
