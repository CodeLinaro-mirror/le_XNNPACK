...
template <>
class NumericLimits<qcint32> {
 public:
  static int32_t min() { return -10000; }
  static int32_t max() { return 10000; }
  static int32_t smallest_normal() { return 0; }
  static int32_t min_identity() { return max(); }
  static int32_t max_identity() { return min(); }
};
...
void MatrixVectorMultiply(const Input* input, const Tensor<Filter>& filter,
  ...
  for (size_t oc = 0; oc < filter.extent(0); ++oc) {
    double output_ic =
        bias.empty()
            ? 0.0f
            : dequantize(bias(oc), {bias_zero_point, bias_scale(oc, 0)});
...
DatatypeGenerator<qcint32> MakeDatatypeGenerator(qcint32) {
  // We need to trick DatatypeGenerator into _not_ creating a "reinterpret"
  // number generator.
  return DatatypeGenerator<qcint32>(NumericLimits<qcint32>::min() + 1,
                                    NumericLimits<qcint32>::max() - 1);
}
...
xnn_quantization_params CalculateFullyConnectedQuantizationParams(
  ...
  const float bias_min =
      dequantize(NumericLimits<Bias>::min(), bias_quantization);
  const float bias_max =
      dequantize(NumericLimits<Bias>::max(), bias_quantization);
...
void TestStaticB(xnn_datatype convert_to = xnn_datatype_invalid,
  ...
  for (auto _ : FuzzTest(std::chrono::milliseconds(500))) {
    ...
    // quantization.
    xnn_quantization_params bias_quantization =
        xnn_datatype_is_quantized(datatype_of<Output>())
    ...
    for (int reshape = 0; reshape < 2; ++reshape) {
      ...
      Tensor<Scale> bias_scale({bias.size(), 1});
      if (xnn_datatype_is_channelwise_quantized(datatype_of<Bias>())) {
        bias_zero_point = filter_quantization.zero_point;
        for (size_t k = 0; k < bias_scale.size(); k++) {
          bias_scale(k, 0) = input_quantization.scale * filter_scale(k, 0);
        }
      } else {
...
void TestDynamicB(xnn_datatype convert_to = xnn_datatype_invalid,
  ...
  for (auto _ : FuzzTest(std::chrono::milliseconds(500))) {
    ...
    for (int reshape = 0; reshape < 2; ++reshape) {
      ...
      // Verify results.
      Tensor<float> bias_scale({bias.size(), 1});
      bias_scale.fill(bias_quantization.scale);
