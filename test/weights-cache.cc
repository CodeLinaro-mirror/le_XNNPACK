...
TEST(WEIGHTS_CACHE, get_or_insert)
{
  ...
  ASSERT_EQ(0, xnn_internal_get_or_insert_weights_cache(&cache, cache.cache.weights.start, 4));
  ...
  write_weights(&cache, "1234");
  ASSERT_EQ(0, xnn_internal_get_or_insert_weights_cache(&cache, span2_weights, 4));
  ...
  ASSERT_EQ(4, xnn_internal_get_or_insert_weights_cache(&cache, span3_weights, 4));
  ASSERT_EQ(1, cache.cache.hits);
...
TEST(WEIGHTS_CACHE, grow) {
  ...
  for (size_t i = 0, expected_offset = 0; i < old_num_buckets; i++) {
    ...
    ASSERT_EQ(expected_offset, xnn_internal_get_or_insert_weights_cache(&cache, cache_end(&cache), s.length()));
...
TEST(WEIGHTS_CACHE, write_many_cache_hits) {
  ...
  auto write = [&] {
    ...
    xnn_internal_get_or_insert_weights_cache(&cache, cache_end(&cache), weights_size);
  };
...
TEST(WEIGHTS_CACHE, operations_on_finalized_cache_hard) {
  ...
  ASSERT_EQ(XNN_CACHE_NOT_FOUND, xnn_internal_get_or_insert_weights_cache(&cache, cache.cache.weights.start, 4));
...
TEST(WEIGHTS_CACHE, insert_into_finalized_cache_soft) {
  ...
  ASSERT_EQ(0, xnn_internal_get_or_insert_weights_cache(&cache, cached_weights, 4));
  ASSERT_EQ(4, cache.cache.weights.size);

  // Sufficient space, but Cache miss.
  write_weights(&cache, "4567");
  ASSERT_EQ(XNN_CACHE_NOT_FOUND, xnn_internal_get_or_insert_weights_cache(&cache, cached_weights, 4));
