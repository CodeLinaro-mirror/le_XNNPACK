...
#define YNN_UNARY_REDUCE_KERNEL(arch_flags, name, type_a, type_c) \
  KernelInfo{arch_flags, name, #name, ynn_reduce_max,             \
             multi_type_of(type_a(), type_c())},
#include "ynnpack/kernels/reduce/max.inc"
#undef YNN_UNARY_REDUCE_KERNEL

#define YNN_UNARY_REDUCE_KERNEL(arch_flags, name, type_a, type_c) \
  KernelInfo{arch_flags, name, #name, ynn_reduce_min,             \
             multi_type_of(type_a(), type_c())},
...
void TestReduce(AT, CT, ynn_reduce_operator op) {
  ...
  const ReduceShape shapes[] = {
      {16, 256, 1, 1}, {1, 33, 5, 7},   {128, 17, 1, 1},
      {128, 32, 4, 1}, {64, 1, 1, 128},
  };

  using Layout = std::vector<int32_t>;
  ...
  for (const ReduceShape& shape : shapes) {
    ...
    for (const Layout& layout : layouts) {
      ...
      for (const KernelInfo& kernel : all_kernels) {
        ...
        // TODO(ashaposhnikov): Make the remaining kernels consistent with
        // AVX/AVX512.
