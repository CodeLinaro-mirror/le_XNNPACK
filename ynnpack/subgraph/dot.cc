...
std::tuple<index_t, index_t, index_t> choose_split_factors(index_t m, index_t n,
  ...
  const index_t effective_k = std::min<index_t>(k, 1024);
  ...
  split_k = align_up<index_t>(split_k, 1024);
  ...
  constexpr index_t max_mult_n = 2047;  // 2^11 - 1
  constexpr index_t max_mult_m = 2047;  // 2^11 - 1
  constexpr index_t max_mult_k = 511;   // 2^9 - 1

  split_n = std::min<index_t>(split_n, max_mult_n * block_n);
  split_m = std::min<index_t>(split_m, max_mult_m * 16);
  split_k = std::min<index_t>(split_k, max_mult_k * 1024);
...
std::tuple<slinky::expr, slinky::expr, slinky::expr> choose_split_factors(
  ...
  // We can only return a single scalar from a slinky expression, so we pack the
  // split factors into one 32-bit integer using:
  //   splits = (mult_k * 2048 + mult_m) * 2048 + mult_n
  //
  ...
  // = block_n).
  //   Max representable split_n: 2047 * block_n (e.g. 131,008 for block_n =
  //   64).
  // - mult_m: 11 bits (bits 11-21, [0, 2047]), multiplied by 16 (step size =
  // 16).
  //   Max representable split_m: 2047 * 16 = 32,752.
  // - mult_k: 9 bits (bits 22-30, [0, 511]), multiplied by 1024 (step size =
  // 1024).
  //   Max representable split_k: 511 * 1024 = 523,264.
  //
  // Maximum packed value: (511 * 2048 + 2047) * 2048 + 2047 = 2,147,483,647
  // (INT32_MAX).
  constexpr index_t kRadixN = 2048;  // 2^11
  constexpr index_t kRadixM = 2048;  // 2^11
  auto impl = [=](const slinky::call* op,
    ...
    index_t mult_m = std::max<index_t>(1, split_m / 16);
    index_t mult_k = std::max<index_t>(1, split_k / 1024);
    return (mult_k * kRadixM + mult_m) * kRadixN + mult_n;
  };
  ...
  slinky::expr mult_n = splits % kRadixN;
  slinky::expr splits_m_k = splits / kRadixN;
  slinky::expr mult_m = splits_m_k % kRadixM;
  slinky::expr mult_k = splits_m_k / kRadixM;
  slinky::expr split_n = slinky::min(n, mult_n * block_n);
  slinky::expr split_m = slinky::min(m, mult_m * 16);
  slinky::expr split_k = slinky::min(k, mult_k * 1024);
...
ynn_status define_dot(ynn_subgraph& subgraph, size_t num_k_dims,
  ...
  node.create = [consistent_arithmetic, symmetric_b, pack_b, transpose_a,
    ...
    const auto const_m = as_constant(m);
    const auto const_n = as_constant(n);
    const auto const_k = as_constant(k);
    const auto const_block_n = as_constant(block_n);
    if (const_m && const_n && const_k && const_block_n) {
      std::tie(split_n, split_m, split_k) =
          choose_split_factors(*const_m, *const_n, *const_k, *const_block_n,
                               type_size_bytes(input_a.type));
      split_m = runtime.globals.get(split_m, "split_m");
      split_n = runtime.globals.get(split_n, "split_n");
      split_k = runtime.globals.get(split_k, "split_k");
    } else {
