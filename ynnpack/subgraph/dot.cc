...
const cpu_info& get_cpu_info() {
  static const cpu_info info = []() {
    cpu_info info;
    info.cache_sizes.l1 = default_cache_size_l1;
    info.cache_sizes.l2 = default_cache_size_l2;
    info.cache_sizes.l3 = default_cache_size_l3;
...
auto make_dot_impl(dot_type type, bool consistent_arithmetic, bool transposed_a,
  ...
  return [type, consistent_arithmetic, transposed_a, pack_b, num_k_dims](
    ...
    dot_loop loops_storage[3 * 3];
    const cpu_info& cpu_info = get_cpu_info();

    if (k1) {
      auto loops =
          schedule_dot(cpu_info, c_m.extent(), c_n.extent(), k, block_m,
                       block_n, block_k, a.elem_size, b.elem_size, c.elem_size,
                       pack_b, transposed_a, loops_storage);
    ...
    if (k1_tail) {
      std::array<size_t, 3> k_tail = {static_cast<size_t>(k1_tail),
                                      ...
                                      static_cast<size_t>(k3)};
      auto loops =
          schedule_dot(cpu_info, c_m.extent(), c_n.extent(), k_tail, block_m,
                       block_n, block_k, a.elem_size, b.elem_size, c.elem_size,
                       pack_b, transposed_a, loops_storage);
...
// 1. 16MB Fast Path: Extremely small workloads (<= 16MB) bypass the thread pool
//    entirely, resolving in a single thread to eliminate OS scheduling
//    overhead.
// 2. Asymmetric Aspect Scaling: For asymmetric shapes (M-heavy or N-heavy), the
//    maximum task area is geometrically bounded by the cube of the major
//    dimension (e.g., (m * n * n) / 1024) to keep slices uniformly balanced and
//    prevent skinny slivers.
...
std::tuple<slinky::expr, slinky::expr> choose_split_factors(
  ...
  auto impl = [elem_a, elem_b, elem_c](const slinky::call* op,
    ...
    // Extremely small workloads (<= 16MB) bypass the thread pool entirely.
    if (m * n * k * elem_a <= 16 * 1024 * 1024 && m < 32768 && n < 65536) {
      return m * 65536 + n;
    }

    // Cap K to 1024. If K is excessively large, it will be tiled anyway in
    // `schedule_dot`. Capping it here ensures we don't overestimate the cost of
    // a chunk.
    k = std::min<index_t>(k, 1024);

    // Considerations for task size:
    // - Footprint: The total memory working set size in bytes. We target a
    //   specific footprint to maximize data reuse and saturate CPU caches.
    // - Cost: The total number of input elements read from matrices A and B. It
    //   acts as a proxy for memory bandwidth and data loading cost.
    index_t min_footprint = 256 * 1024;
    index_t max_footprint = 512 * 1024;
    index_t target_cost = 64 * 1024;
    ...
    if (aspect_ratio != 4) {
      // Highly asymmetric matrices suffer from extreme thread starvation if
      // chopped too finely. We boost `max_footprint` and `target_cost` to
      // coalesce tiny slivers into fewer, denser blocks.
      max_footprint *= 2;
      target_cost *= 2;
    }
