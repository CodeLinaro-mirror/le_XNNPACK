...
auto make_dot_impl(dot_type type, bool consistent_arithmetic, bool transposed_a,
  ...
  return [type, consistent_arithmetic, transposed_a, pack_b, num_k_dims](
    ...
    if (a_stride_m == a_stride_k1) {
      // If the stride of m and k1 is the same (i.e. A is a vector of tile_k
...
ynn_status define_dot(ynn_subgraph& subgraph, size_t num_k_dims,
  ...
  node.create = [consistent_arithmetic, pack_b, transpose_a, block_n_unpacked](
    ...
    // two dimensions.
    const int num_b_k_dims = num_k_dims + 3;
    slinky::box_expr b_bounds(num_b_k_dims);
    ...
    for (size_t i = 4; i < num_b_k_dims; ++i) {
