...
void RunFuseCompare(
    ...
    const std::vector<std::vector<A>>& input_datas,
    const std::vector<ynn::TensorShape>& input_shapes, uint32_t output_id,
    size_t output_size,
...
TEST(fusion_lut, single_node_simple) {
  ...
  RunFuseCompare<int8_t, int8_t>(
      builder, {x_id}, {input_data}, {ynn::TensorShape({input_data.size()})},
      y_id, 256, [&](const ynn_subgraph& subgraph) {
...
TEST(fusion_lut, elu_chain) {
  ...
  RunFuseCompare<uint8_t, uint8_t>(
      builder, {x_id}, {input_data}, {ynn::TensorShape({4, 8, 8})}, y_id, 256,
...
TEST(fusion_lut, branching_unsupported) {
  // x -> (abs) -> t -> (ceil) -> y
  //                 \ -> (floor) -> z.
  uint32_t x_id = 0;
  uint32_t t_id = 1;
  ...
  FuseAndCheck(builder, [&](const ynn_subgraph& subgraph) {
    EXPECT_GE(valid_node_count(&subgraph), 3);

    const ynn_node* t_producer = subgraph.get_producer(t_id);
    ASSERT_NE(t_producer, nullptr);
    const ynn_node::unary_elementwise* abs_op =
        std::get_if<ynn_node::unary_elementwise>(&t_producer->op);
    ASSERT_NE(abs_op, nullptr);
    EXPECT_EQ(abs_op->op, ynn_unary_abs);

    const ynn_node* y_producer = subgraph.get_producer(y_id);
    ASSERT_NE(y_producer, nullptr);
    ASSERT_FALSE(is_lut(*y_producer));
    const ynn_node::unary_elementwise* ceil_op =
        std::get_if<ynn_node::unary_elementwise>(&y_producer->op);
    ASSERT_NE(ceil_op, nullptr);
    EXPECT_EQ(ceil_op->op, ynn_unary_ceil);

    const ynn_node* z_producer = subgraph.get_producer(z_id);
    ASSERT_NE(z_producer, nullptr);
    ASSERT_FALSE(is_lut(*z_producer));
    const ynn_node::unary_elementwise* floor_op =
        std::get_if<ynn_node::unary_elementwise>(&z_producer->op);
    ASSERT_NE(floor_op, nullptr);
    EXPECT_EQ(floor_op->op, ynn_unary_floor);
  });
}
...
TEST(fusion_lut, input_type_unsupported) {
  ...
  static const uint8_t b_val[] = {1};

  builder.AddInput(ynn_type_fp32, {256}, x_id)
      .AddTensor(ynn_type_uint8, {256}, a_id)
      .AddTensor(ynn_type_uint8, {1}, b_id, b_val)
      .AddOutput(ynn_type_uint8, {256}, y_id);

  builder.AddUnary(ynn_unary_convert, x_id, a_id)
      .AddBinary(ynn_binary_add, a_id, b_id, y_id);

  FuseAndCheck(builder, [&](const ynn_subgraph& subgraph) {
    EXPECT_GE(valid_node_count(&subgraph), 2);
    ...
    ASSERT_NE(y_producer, nullptr);
    ASSERT_FALSE(is_lut(*y_producer));
    const ynn_node::binary_elementwise* add_op =
        std::get_if<ynn_node::binary_elementwise>(&y_producer->op);
    ASSERT_NE(add_op, nullptr);
    EXPECT_EQ(add_op->op, ynn_binary_add);
  });
}
